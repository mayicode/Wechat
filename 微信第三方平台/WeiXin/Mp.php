...
//				var_dump($encryptStr);
    ...
    public function getUnauthAppid()
    {
        if (isset($this->_receive['AuthorizerAppid'])) {
            $this->authorizer_appid = $this->_receive['AuthorizerAppid'];
	...
	/**
     * 获取微信服务器发来的AuthorizationCode
     * Example: $obj->getRev()->getAuthorizationCode();
     */
    public function getAuthorizationCode()
    {
        if (isset($this->_receive['AuthorizationCode'])) {
            $this->AuthorizationCode = $this->_receive['AuthorizationCode'];
	...
	/**
	 * 获取微信服务器发来的InfoType
	 * Example: $obj->getRev()->getInfoType();
	 */
	public function getInfoType()
	{
		if (isset($this->_receive['InfoType'])) {
			$this->InfoType = $this->_receive['InfoType'];
 ...
 */
    public function checkComponentAuth($component_appid = '', $component_appsecret = '')
    {
		...
		$data = json_decode(file_get_contents("wxoauthml/component_access_token.json"));
		if ($data->expires_in < time()) {
			...
			$result = $this->http_post(self::API_URL_PREFIX . self::COMPONENT_AUTH_URL, self::json_encode($c_data));
			if ($result) {
				...
				if ($this->component_access_token) {
					...
					$fp = fopen("wxoauthml/component_access_token.json", "w");
    ...
    public function getAuthRefreshToken($component_appid = '')
    {
        ...
        if ($result) {
			...
			if(!empty($func_info_arr)){
				foreach($func_info_arr as $func_a){
					$func_info .= $func_a['funcscope_category']['id'].',';
				}
